...
class _CalendarioHorarioState extends State<CalendarioHorario> {
      void _showTareaActionsDialog(BuildContext context, HorarioItemEntity tarea) async {
        showDialog(
          context: context,
          builder: (ctx) => SimpleDialog(
            ...
            children: [
                ...
                child: Row(
                  children: [
                    Icon(
                      tarea.completado ? Icons.check_box : Icons.check_box_outline_blank,
                      color: Colors.green,
                    ),
                    const SizedBox(width: 8),
                    Text(tarea.completado ? 'Marcar como pendiente' : 'Marcar como hecha'),
                  ],
                ),
              ),
              SimpleDialogOption(
  ...
  TextStyle get _headerTextStyle => TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: widget.primaryColor);
  TextStyle get _dowTextStyle => const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Colors.black87);
  TextStyle get _dayNumberStyle => const TextStyle(fontWeight: FontWeight.w700);
  TextStyle get _eventTitleStyle => const TextStyle(fontSize: 13, fontWeight: FontWeight.w700);
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        // Card-like container for the calendar to match app style
        Container(
          ...
          child: TableCalendar<HorarioItemEntity>(
            ...
            calendarStyle: CalendarStyle(
              todayDecoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(color: widget.accentColor.withOpacity(0.12), blurRadius: 6)],
              ),
              selectedDecoration: BoxDecoration(
                color: widget.primaryColor,
                shape: BoxShape.circle,
                boxShadow: [BoxShadow(color: widget.primaryColor.withOpacity(0.22), blurRadius: 8)],
              ),
            ...
            daysOfWeekStyle: DaysOfWeekStyle(
              ...
              weekendStyle: _dowTextStyle.copyWith(color: Colors.redAccent),
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            Text('Eventos de la semana ${startOfWeek.day}/${startOfWeek.month} - ${startOfWeek.add(Duration(days:6)).day}/${startOfWeek.add(Duration(days:6)).month}',
              ...
              return Column(
                ...
                children: [
                  Padding(
                    ...
                    child: Text('${_diasSemana[d.weekday - 1]} ${d.day}/${d.month}', style: const TextStyle(fontWeight: FontWeight.w700)),
                  ),
                  if (events.isEmpty)
                    Padding(
                      padding: const EdgeInsets.only(bottom: 8.0, left: 6.0),
                      child: Text('No hay eventos', style: TextStyle(color: Colors.grey[600])),
                    ),
                  ...events.map((e) => Card(
                        margin: const EdgeInsets.symmetric(vertical: 4),
                        child: ListTile(
                          leading: Container(width: 10, height: 10, decoration: BoxDecoration(color: e.color, borderRadius: BorderRadius.circular(3))),
                          title: Text(e.titulo, style: const TextStyle(fontWeight: FontWeight.w600)),
                          subtitle: Text('${e.tipo} • ${e.inicio.hour.toString().padLeft(2,'0')}:${e.inicio.minute.toString().padLeft(2,'0')}'
                              + (e.fin != null ? ' - ${e.fin!.hour.toString().padLeft(2,'0')}:${e.fin!.minute.toString().padLeft(2,'0')}' : '')),
                          trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                            if (e.completado) const Icon(Icons.check, color: Colors.green),
                            if (widget.onEliminar != null)
                              IconButton(
                          ...
                          onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => HorarioItemDetailScreen(item: e))),
  ...
  Widget _buildDayCell(DateTime day, List<HorarioItemEntity> events, bool isToday, {bool selected = false}) {
    return ClipRect(
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          // Allocate space: circle ~50% of height, title ~30%, small gaps
          final circleSize = (maxH * 0.5).clamp(18.0, 28.0);
          final titleMaxWidth = (maxW * 0.85).clamp(48.0, 140.0);
          ...
          return InkWell(
            onTap: () {
              ...
              setState(() {
                _selectedDay = day;
                _focusedDay = day;
                if (widget.onDiaSeleccionado != null) widget.onDiaSeleccionado!(day);
              });
            },
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    color: selected
                        ...
                        : isToday
                            ...
                            : Colors.grey[100],
                    boxShadow: selected
                        ? [BoxShadow(color: widget.primaryColor.withOpacity(0.25), blurRadius: 8, offset: Offset(0, 2))]
                        : [],
                  ),
                  ...
                  child: Text(
                    ...
                    style: _dayNumberStyle.copyWith(
                      color: selected ? Colors.white : (isToday ? widget.primaryColor : Colors.black87),
                      fontSize: (circleSize * 0.5).clamp(12.0, 16.0),
                    ),
                  ),
                ),
                if (events.isNotEmpty)
                  Container(
                    constraints: BoxConstraints(maxWidth: titleMaxWidth, minHeight: 16),
                    decoration: BoxDecoration(
                      color: selected ? widget.primaryColor.withOpacity(0.9) : widget.accentColor.withOpacity(0.18),
                    ...
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Container(width: 8, height: 8, decoration: BoxDecoration(color: events.first.color, borderRadius: BorderRadius.circular(4))),
                        const SizedBox(width: 6),
                        Flexible(
                          child: Text(
                            events.first.titulo,
                            style: _eventTitleStyle.copyWith(
                              color: events.first.completado ? Colors.green[700] : (selected ? Colors.white : Colors.black87),
                              decoration: events.first.completado ? TextDecoration.lineThrough : null,
                            ),
                ...
                if (events.length > 1)
                  Padding(
                    ...
                    child: Text('...', style: TextStyle(fontSize: 14, color: widget.accentColor, fontWeight: FontWeight.bold)),
                  ),
              ],
            ),
          );
  ...
  void _showDaySummaryDialog(BuildContext context, DateTime day, List<HorarioItemEntity> events) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
          ...
          child: events.isEmpty
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      leading: CircleAvatar(backgroundColor: e.color, radius: 12),
                      title: Text(e.titulo, style: TextStyle(decoration: e.completado ? TextDecoration.lineThrough : null)),
                      subtitle: Text(
                      ...
                      onTap: () {
                        ...
                        Navigator.of(context).push(MaterialPageRoute(builder: (_) => HorarioItemDetailScreen(item: e)));
                      },
                    );
                  },
                ),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(ctx).pop();
              Navigator.of(context).push(MaterialPageRoute(builder: (_) => HorarioItemFormScreen(initialDate: day)));
            },
  ...
  Future<bool> _confirmDelete(BuildContext context, HorarioItemEntity item) async {
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Eventos para ${day.day}/${day.month}/${day.year}:', style: const TextStyle(fontWeight: FontWeight.bold)),
            if (events.isEmpty)
              const Padding(
                  ...
                  child: ListTile(
                    leading: Checkbox(
                      ...
                      onChanged: widget.onToggleCompletado == null
                          ? null
                          : (val) => widget.onToggleCompletado!(e, val ?? false),
                    ),
                    title: Text(
                      e.titulo,
                      style: TextStyle(
                        decoration: e.completado ? TextDecoration.lineThrough : null,
                      ),
                    ),
                    subtitle: Text(
                      '${e.tipo} • ${e.inicio.hour.toString().padLeft(2, '0')}:${e.inicio.minute.toString().padLeft(2, '0')}'
